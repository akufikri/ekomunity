            ...
            <div class="card-header">
                ...
                @if (Auth::user()->sub_company == null)
                    <div class="card-tools">
                        <a href="#" data-toggle="modal" title="Add" data-target="#addShareholders"
...
<div class="modal fade" id="addShareholders" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body my-table">
                <!-------->
                <form action="{{ URL::to('/maklumatJawatanKuasa/store') }}#view" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputEmail1">Jawatan</label>
                        <select class="form-control form-control-sm" name="id_position" id="id_position" required
                            autofocus>
                            <option selected disabled value="">Pilih Jawatan</option>
                            @foreach ($position as $data)
                                <option value="{{ $data->id_position }}"
                                    {{ old('id_position') == $data->id_position ? '' : '' }}>
                                    {{ $data->position }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="datetime-local" name ="date_appointment" id="date_appointment"
...
<div class="modal fade" id="updateShareholders" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body my-table">
                <!-------->
                <form action="{{ URL::to('/shareholders') }}#view" class="form-update-data" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputEmail1">Jawatan</label>
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1">Nama</label>
                        <select class="form-control form-control-sm" name="id_user" id="id_user" required
                            autofocus>
                            <option selected disabled value="">Pilih Nama</option>
                            @foreach ($ahli as $data)
                                <option value="{{ $data->manpower->user->id }}"}}>
            ...
            columnDefs: [{
                ...
                "render": function(data, type, row) { //class="btn btn-primary btn-sm"
                    var btn = //'<a href="/learn/detail/'+row.id_company_shareholders+'" data-toggle="modal" data-target="#addPosition" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a>'+
                        '<a href="/update_position/' + row.id_company_shareholders +
                        '" id="editData" data-id="' + row.id_manpower_position +
                        '"  data-toggle="modal" data-target="#updateShareholders" class="btn btn-warning btn-sm" ><i class="fa fa-edit nav-icon"></i></a> ' +
                        '<a href="" id="deleteData" data-id="' + row.id_manpower_position +
                        '" class="btn btn-danger btn-sm"><i class="fa fa-trash nav-icon"></i></a>'
    ...
    function loadDataTemp() {
        $('#datatable-crud-temp').DataTable({
            ...
            columnDefs: [{
                ...
                "render": function(data, type, row) { //class="btn btn-primary btn-sm"
                    var btn = //'<a href="/learn/detail/'+row.id_company_shareholders+'" data-toggle="modal" data-target="#addPosition" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a>'+
                        '<a href="" id="deleteDataTemp" data-id="' + row
                        .id_temp_company_shareholders +
                        '" class="btn btn-danger btn-sm" ><i class="fa fa-trash nav-icon"></i></a>'
                    return btn; //settings_position/'+row.id_position+'/edit
                }
            }, ],
