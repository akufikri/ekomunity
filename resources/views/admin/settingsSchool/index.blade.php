...
<li class="breadcrumb-item active"><a>Settings City</a></li>
@endsection
    ...
    <div class="col-12">
        <div class="card card-primary card-outline" style="border-top: 3px solid dark">
            <div class="card-header">
                <h3 class="card-title text-primary my-header">Settings School</h3>
                <div class="card-tools">
                    <a href="#" data-toggle="modal" data-target="#addSchool" class="btn btn-sm btn-success pull-right">
...
<div class="modal fade" id="addSchool" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('settings_school.create')}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                ...
                <div class="modal-body my-table">
                    ...
                    <label for="exampleInputEmail1">From School Type</label>
                    <select class="form-control form-control-sm" name="id_type_school" id="id_type_school" required autofocus>
                      <option selected disabled value="">Select School Type</option>
                      @foreach(\App\Models\TypeSchool::get() as $data)
                      <option value="{{$data->id_type_school}}" {{(old('id_type_school')==$data->id_type_school)? :''}}>
                        {{$data->type_school}}
                      </option>
                      @endforeach
                    </select>
    ...
    <div class="modal-dialog" role="document">
        <form action="" method="POST" class="form-update-data" enctype="multipart/form-data">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body my-table">
                    <div class="form-group" >
                    <input type="hidden" id="id_school" name="id_school">
...
<!--DETAIL SCHOOL-->
<div class="modal fade" id="detailSchool" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="" method="POST" class="form-update-data" enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                ...
                <div class="modal-body my-table">
                    <div class="form-group" >
                    <label for="exampleInputEmail1">School Type</label>
                    <select class="form-control form-control-sm" name="id_type_school" id="id_type_school" required disabled>
                      <option selected disabled value="">Select School Type</option>
                      @foreach(\App\Models\TypeSchool::get() as $data)
                      <option value="{{$data->id_type_school}}" {{(old('id_type_school')==$data->id_type_school)? :''}}>
                        {{$data->type_school}}
                ...
                <div class="form-group">
                        <label for="exampleInputEmail1">School Name</label>
                        <input type="text" class="form-control" id="school" name="school" placeholder="School Name" readonly>
                    </div>
                <div class="form-group">
                        <label for="exampleInputEmail1">Description</label>
...
<script type="text/javascript">
    ...
    function loadData(){
        $('#datatable-crud').DataTable({
            ...
            scrollX: true,
            ajax:{
                url: "{{ route('settings_school.index') }}",
                type: 'GET'
            },
            columns: [
                { data: 'DT_RowIndex'},
                { data: 'type_school_name'},
                { data: 'school'},
                { data: 'description'},
                { data: 'is_active'},
                { data: 'create_date'},
            ...
            columnDefs: [
                {
                   ...
                   "render": function(data, type, row){
                       var btn = '<a href="#" id="detailData" data-id="'+row.id_school+'" data-toggle="modal" data-target="#addSchool" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a>'+
                       '<a href="#" data-toggle="modal" id="editData" data-id="'+row.id_school+'" data-target="#updateSchool" class="btn btn-warning btn-sm"><i class="fa fa-edit nav-icon"></i></a>'
            ...
            $.ajax(
            {
                ...
                success: function (response){
                    ...
                    if (response.data != null) {
                        ...
                        form.find('input[name=id_school]').val(data.id_school);
                        form.find('select[name=id_type_school]').val(data.id_type_school);
    ...
    $(document).on('submit', '.form-update-data', function(e){
            ...
            var ini = $(this),  input_token = $('input[name=_token]'),
                id = ini.find('input[name=id_school]').val(),
                url = '/update_school/'+id;
            var post_data = {
                is_ajax: true,
                _token: input_token.val(),
                id_type_school: ini.find('select[name=id_type_school]').val(),
                school: ini.find('input[name=school]').val(),
