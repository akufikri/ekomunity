...
<li class="breadcrumb-item active"><a>Settings Position (Shareholders)</a></li>
@endsection
    ...
    <div class="col-12">
        <div class="card card-primary card-outline" style="border-top: 3px solid dark">
            <div class="card-header">
                <h3 class="card-title text-primary my-header">Settings Position (shareholders)</h3>
                <div class="card-tools">
                    <a href="#" data-toggle="modal" data-target="#addPosition"  class="btn btn-sm btn-success pull-right">
...
<!--ADD POSITION-->
        <div class="modal fade" id="addPosition" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <form action="{{ route('settings_position_jabatan.create') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<!--UPDATE POSITION-->
        <div class="modal fade" id="updatePosition" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
...
<!--DETAIL POSITION-->
        <div class="modal fade" id="detailPosition" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
            <form action="" class="form-update-data" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Detail Position</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
...
$(document).ready( function () {
    ...
    $('#datatable-crud').DataTable({
        ...
        scrollX: true,
        ajax: {
                url: "{{ route('settings_position_jabatan.index') }}",
                type: 'GET'
            },
        columns: [
            { data: 'DT_RowIndex'},
        ...
        columnDefs: [
                {
                    ...
                    "render" : function (data, type, row) {//class="btn btn-primary btn-sm"
                        var btn = '<a href="#" data-toggle="modal" data-target="#detailPosition" data-id="'+row.id_position_jabatan+'" id="detailData" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a>'+
                        '<a href="#" id="editData" data-id="'+row.id_position_jabatan+'"  data-toggle="modal" data-target="#updatePosition" class="btn btn-warning btn-sm"><i class="fa fa-edit nav-icon"></i></a>'
                        // '<a href="" id="deleteData" data-id="'+row.id_position_jabatan+'" class="btn btn-danger btn-sm"><i class="fa fa-trash nav-icon"></i></a>'
                        return btn; //settings_position_jabatan/'+row.id_position_jabatan+'/edit
                    }
                },
            ],
        order: [[0, 'asc']]
    ...
    $("body").on("click","#editData",function(e){
            ...
            $.ajax(
            {
                url: "/settings_position_jabatan/"+id+"/edit", //settings_position_jabatan/'+row.id_position_jabatan+'/edit
                type: 'GET',
                data: {
                    _token: token,
                        id: id
                ...
                success: function (response){
                    ...
                    if (response.data != null) {
                        ...
                        form.find('input[name=id]').val(data.id_position_jabatan);
    ...
    $(document).on('submit', '.form-update-data', function(e){
            ...
            var ini = $(this),  input_token = $('input[name=_token]'),
                ...
                url = '/update_position_jabatan/'+id;
            var post_data = {
                is_ajax: true,
                _token: input_token.val(),
                position: ini.find('input[name=position]').val(),
    ...
    $("body").on("click","#deleteData",function(e){
            ...
            $.ajax(
            {
                url: "/settings_position_jabatan/"+id, 
                type: 'DELETE',
                data: {
                    _token: token,
                        id: id
