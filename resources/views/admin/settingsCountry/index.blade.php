    ...
    <div class="col-12">
        <div class="card card-danger card-outline" style="border-top: 3px solid dark">
            <div class="card-header">
                <h3 class="card-title text-danger my-header">Country</h3>
...
<!--UPDATE Country-->
        <div class="modal fade" id="updateCountry" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
            <form action="" class="form-update-data" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    ...
                    <div class="form-group">
                        <label for="country_name">Country Name</label>
                        <input type="hidden" class="form-control" id="id_country" name="id_country" placeholder="id" hidden>
                        <input type="text" class="form-control" id="country_name" name="country_name" placeholder="Country Name" required>
                    </div>
                    <div class="form-group">
                        <label for="country_code">Country Code</label>
...
<!--DETAIL Country-->
        <div class="modal fade" id="detailCountry" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
            <form action="" class="form-update-data" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    ...
                    <div class="form-group">
                        <label for="country_name">Country Name</label>
                        <input type="hidden" class="form-control" id="id_country" name="id_country" placeholder="id" hidden>
                        <input type="text" class="form-control" id="country_name" name="country_name" placeholder="Country Name" readonly>
                    </div>
                    <div class="form-group">
                        <label for="country_code">Country Code</label>
                        <input type="text" class="form-control" id="country_code" name="country_code" placeholder="Country Code" readonly>
                    </div>
                    <div class="form-group">
                        <label for="description">Description</label>
...
function loadData(){
    $('#datatable-crud').DataTable({
        ...
        destroy: true,
        ajax: {
                url: "{{ route('settings_country.index') }}",
                type: 'GET'
            },
        columns: [
            { data: 'DT_RowIndex' },
        ...
        columnDefs: [
                ...
                },
                {
                    ...
                    "data": "c",
                    "render" : function (data, type, row) {//class="btn btn-primary btn-sm"
                        var btn = '<a href="#" data-toggle="modal" id="detailData" data-id="'+row.id_country+'" data-target="#detailCountry" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a> '+
                        '<a href="#" id="editData" data-id="'+row.id_country+'" data-toggle="modal" data-target="#updateCountry" class="btn btn-warning btn-sm"><i class="fa fa-edit nav-icon"></i></a>'
...
$(document).ready(function(){
            ...
            $.ajax(
            {
                ...
                success: function (response){
                    ...
                    if (response.data != null) {
                        ...
                        form.find('input[name=id_country]').val(data.id_country);
                        form.find('input[name=country_name]').val(data.country_name);
                        form.find('input[name=country_code]').val(data.country_code);
    ...
    $(document).on('submit', '.form-update-data', function(e){
            ...
            var ini = $(this),  input_token = $('input[name=_token]'),
                id = ini.find('input[name=id_country]').val(),
                url = '/update_country/'+id;
            var post_data = {
                is_ajax: true,
                _token: input_token.val(),
                country_name: ini.find('input[name=country_name]').val(),
                country_code: ini.find('input[name=country_code]').val(),
