...
<li class="breadcrumb-item active">Settings Qualification</li>
    ...
    <div class="col-12">
        <div class="card card-primary card-outline" style="border-top: 3px solid dark">
            <div class="card-header">
                <h3 class="card-title text-primary my-header">Settings Qualification</h3>
...
<!--ADD Qualification-->
        <div class="modal fade" id="addQualification" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <form action="{{ route('settings_qualification.create') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<!--UPDATE Qualification-->
        <div class="modal fade" id="updateQualification" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
            <form action="" class="form-update-data" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Update Qualification</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body my-table">
                        <!-------->
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="qualification" name="qualification" placeholder="Qualification Name" required>
                          </div>
                          <div class="form-group">
                            <label for="exampleInputEmail1">Description</label>
                            <input type="text" class="form-control" id="description" name="description" placeholder="Description" required>
...
<!--DETAIL Qualification-->
        <div class="modal fade" id="detailQualification" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
            <form action="" class="form-update-data" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Detail Qualification</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body my-table">
                        <!-------->
                          <div class="form-group">
                            <label for="qualification">Qualification</label>
                            <input type="hidden" class="form-control" id="id" name="id" placeholder="id">
                            <input type="text" class="form-control" id="qualification" name="qualification" placeholder="Qualification Name" readonly>
                          </div>
                          <div class="form-group">
                            <label for="exampleInputEmail1">Description</label>
...
function loadData(){
    $('#datatable-crud').DataTable({
        ...
        scrollX: true,
        ajax: {
                url: "{{ route('settings_qualification.index') }}",
                type: 'GET'
            },
        columns: [
            { data: 'DT_RowIndex' },
        ...
        columnDefs: [
                /*{
                    ...
                    "render" : function (data, type, row) {
                        return '<img src="'+row.gambar+'" width="150px">';
                    }
                },*/
                {
                    "targets" : 5,
                    ...
                    "render" : function (data, type, row) {//class="btn btn-primary btn-sm"
                        var btn = '<a href="#" data-toggle="modal" id="detailData" data-id="'+row.id_qualification+'" data-target="#detailQualification" class="btn btn-primary btn-sm"><i class="fa fa-info-circle nav-icon"></i></a>'+
                        '<a href="#" id="editData" data-id="'+row.id_qualification+'" data-toggle="modal" data-target="#updateQualification" class="btn btn-warning btn-sm"><i class="fa fa-edit nav-icon"></i></a>'
...
$(document).ready(function(){
    ...
    $("body").on("click","#editData",function(e){
            ...
            $.ajax(
            {
                url: "/settings_qualification/"+id+"/edit",
                type: 'GET',
                data: {
                    _token: token,
                        id: id
                ...
                success: function (response){
                    ...
                    if (response.data != null) {
                        ...
                        form.find('input[name=id]').val(data.id_qualification);
                        form.find('input[name=qualification]').val(data.qualification);
                        form.find('input[name=description]').val(data.description);
                        form.find('select[name=status]').val(data.is_active);
            
                        $('#updateQualification').modal('show');
                    } else {
            ...
            $.ajax(
            {
                ...
                success: function (response){
                    ...
                    if (response.data != null) {
                        ...
                        $('#detailQualification').modal('show');
                    } else {
                        // failedAlert('Not Found');
                    }
                }
    ...
    $(document).on('submit', '.form-update-data', function(e){
            ...
            var ini = $(this),  input_token = $('input[name=_token]'),
                ...
                url = '/update_qualification/'+id;
            var post_data = {
                is_ajax: true,
                _token: input_token.val(),
                qualification: ini.find('input[name=qualification]').val(),
            ...
            .done(function (result) {
                ...
                if (result.data != null) {
                    $('#updateQualification').modal('hide');
                    // initData(param)
                    ...
                    swal(
                        'Success!',
                        'Update Qualification Successfully!',
                        'success'
                    )
