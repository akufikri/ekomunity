...
class CompanyOutSourceController extends Controller
{
    public function __constructor(){
        $this->middleware('company');
    }

    public function index_outSource(Request $request)
    {
        if ($request->ajax()) {
            $user = Auth::user()->id;
            
            $data = CompanyProject::whereHas('source', function($source) {
                $source->where('id_source','2');
        ...
        return view('company.viewOutSourceProject');
    }
    
    public function index_outSource_temp(Request $request)
    {
        if ($request->ajax()) {
            $user = Auth::user()->id;
    ...
    public function index_outSource_temp_admin(Request $request, $id, $id_request)
    {
        if ($request->ajax()) {
    ...
    public function index_outSource_admin($id, Request $request)
    {
        if ($request->ajax()) {
            $user = $id;
    ...
    public function edit($id, Request $r)
    {
        ...
        return view('company.edit_out_project', compact('data','datas'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $data->save();
        return redirect()->to('/companyOutSourceProject')->with('success','Updated successfully'); 
    }
