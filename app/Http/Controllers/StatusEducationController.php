...
class StatusEducationController extends Controller{
    
    public function index(Request $request){
        if($request->ajax()){
            $data = StatusEducation::orderBy('id_status_education', 'desc')->get();
            foreach($data as $d){
                $d->create_date = date('d-m-Y H:i:s', strtotime($d->created_at));
            }
            return Datatables::of($data)->addIndexColumn()->make(true);
        }
        return view('admin.settingsStatusEducation.index');
    }
    
    public function create(Request $request){
        $data = new StatusEducation;
        
        $data->status_education = $request->status_education;
    ...
    {
        $data = StatusEducation::findOrFail($id);
